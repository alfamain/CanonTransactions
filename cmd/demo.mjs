...
const baseline=JSON.parse(fs.readFileSync('replay/source-locked-baseline.json','utf8'));console.log('BASELINE (UNCHANGED SOURCE CONTRACT):',baseline.baseline_result,baseline.source_revision.slice(0,12));
...
for(const [label,input] of [['NO CURRENT EVIDENCE',[{record_id:'expired@1',entity_key:'expired',project:'p',scope:'s',status:'resolved',effective_at:'2026-08-15T00:00:00Z',expires_at:'2020-01-01T00:00:00Z',evidence:'fixture',confidence:'high'}]],['INVALID SCHEMA',[{record_id:'broken@1',entity_key:'broken',project:'p',scope:'s',status:'resolved',effective_at:'not-a-date',evidence:'fixture',confidence:'high'}]]]){const out=resolve(input,{project:'p',scope:'s'});console.log(`${label}:`,out.status,out.reason);}
